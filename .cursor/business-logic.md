...
| CRUD учебных годов | ✓ | ✓ | ✓* | — | — | — | — | — |
| **Классы (ClassGroup)** |
| CRUD классов в школе (таб в настройках/карточке школы) | ✓ | ✓ | ✓ | — | — | — | — | — |
| **Персонал (Staff)** |
...
### Обновление прав при новых фичах

**При появлении новых фич или изменении флоу обязательно обновлять Право / Роль:**

1. **Backend:** добавить разрешение (Permission) и назначить ролям (RolePermission) — через data migration в `users/migrations/` по образцу `0003_seed_permissions.py`.
2. **Документация:** обновить таблицу «Роли и разрешения» и при необходимости «Навигация» в этом файле (business-logic.md).
3. **Frontend:** при новом разделе/странице — добавить пункт в Sidebar и BottomNav с `requiredPermissions` и/или `roles`.
4. **Правило для разработки:** см. `.cursor/rules/update-permissions-on-features.mdc`.
